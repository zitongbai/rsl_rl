...
class CircularBuffer:
    ...
    def reset(self, batch_ids: Sequence[int] | None = None):
        ...
        if batch_ids is None:
            batch_ids = slice(None) # type: ignore
    ...
    def mini_batch_generator(self, fetch_length: int, num_mini_batches: int, num_epochs=8):
        ...
        Use a batch of `self.batch_size * fetch_length` samples, but in different orders in each epoch. 
        In each epoch, there are `num_mini_batches` mini-batches of size `self.batch_size * fetch_length // num_mini_batches`.
        
        Args:
            fetch_length (int): The number of samples to fetch from the buffer
            num_mini_batches (int): The number of mini-batches to create
            num_epochs (int, optional): The number of epochs to run. Defaults to 8.
        """
