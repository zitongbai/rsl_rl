...
class OnPolicyRunnerAMP(OnPolicyRunner):
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        elif self.alg_cfg["class_name"] == "PPOAmp":
            self.training_type = "rl"
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if locs["ep_infos"]:
            for key in locs["ep_infos"][0]:
                ...
                if "/" in key:
                    self.writer.add_scalar(key, value, locs["it"])  # type: ignore
        ...
        for key, value in locs["loss_dict"].items():
            self.writer.add_scalar(f"Loss/{key}", value, locs["it"]) # type: ignore
        ...
        self.writer.add_scalar("Perf/collection time", locs["collection_time"], locs["it"]) # type: ignore
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            if self.alg.rnd:
                self.writer.add_scalar("Rnd/mean_extrinsic_reward", statistics.mean(locs["erewbuffer"]), locs["it"]) # type: ignore
                self.writer.add_scalar("Rnd/mean_intrinsic_reward", statistics.mean(locs["irewbuffer"]), locs["it"]) # type: ignore
                self.writer.add_scalar("Rnd/weight", self.alg.rnd.weight, locs["it"]) # type: ignore
            ...
            # everything else
            self.writer.add_scalar("Train/mean_reward", statistics.mean(locs["rewbuffer"]), locs["it"]) # type: ignore
            self.writer.add_scalar("Train/mean_episode_length", statistics.mean(locs["lenbuffer"]), locs["it"]) # type: ignore
            if self.logger_type != "wandb":  # wandb does not support non-integer x-axis logging
                self.writer.add_scalar("Train/mean_reward/time", statistics.mean(locs["rewbuffer"]), self.tot_time) # type: ignore
